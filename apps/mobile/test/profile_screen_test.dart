...
class TestMockKycRepository implements KycRepository {
  ...
  Future<KycLinkModel> getKycLink() async =>
      KycLinkModel(url: 'https://verify.wewire.com/session-123', stage: 'ONBOARDING');
...
void main() {
  ...
  group('Profile & Account Screen Tests', () {
    ...
    Widget createTestWidget({
      ...
      final activeKycRepo =
          kycStatus != null ? TestMockKycRepository(status: kycStatus) : kycRepo;
    ...
    testWidgets('Renders Profile screen with avatar initials, Kwame Doe name, and email',
    ...
    testWidgets('Renders KYC status Verified badge and Tier 2 text when approved',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(
      ...
      expect(find.text('Tier 2 Verified • Full cross-border wallet & virtual card limits active.'),
          findsOneWidget);
      expect(find.text('View Verification Details'), findsOneWidget);
    });

    testWidgets('Renders KYC Action Required badge and navigates to KYC verification on tap',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget(
        kycStatus: KycStatusModel(
    ...
    testWidgets('Renders Invite a Friend card, displays referral code and copies to clipboard',
        (WidgetTester tester) async {
      await tester.pumpWidget(createTestWidget());
      await tester.pumpAndSettle();
    ...
    testWidgets('Renders Share VessPay card and allows copying share link',
      ...
      // Tap Copy Link
      final copyShareBtn = find.byKey(const Key('profile_copy_share_link_button'));
